...
import {
  CreateDecorrenciaConselhoDto,
  UpdateDecorrenciaConselhoDto,
} from '../dtos';
...
describe('DecorrenciaConselhoController', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        { provide: DecorrenciaConselhoService, useValue: mockService },
      ],
    }).compile();

    controller = module.get<DecorrenciaConselhoController>(
      DecorrenciaConselhoController,
    );
  ...
  describe('when create DecorrenciaConselho', () => {
    it('should create a DecorrenciaConselho and return it', async () => {
      ...
      const createdDecorrenciaConselho = await controller.create(
        DecorrenciaConselho,
      );
  ...
  describe('when update a DecorrenciaConselho', () => {
    it('should update a existing DecorrenciaConselho and return it', async () => {
      ...
      DecorrenciaConselhoUpdate.decorrenciaconselho =
        'Update DecorrenciaConselho ';
      ...
      expect(updatedDecorrenciaConselho).toMatchObject(
        DecorrenciaConselhoUpdate,
      );
      expect(mockService.update).toBeCalledWith('1', DecorrenciaConselhoUpdate);
