...
const getQtdOfWeeks = (init: string, end: string):number => Math.floor(getDiffDateInDays(init, end)/7)
const getMajorDayOfWeek = (init: string, end: string):number => Number(Boolean(new Date(init).getDay()<new Date(end).getDay()))
const getSunday = (init: string):number => Number(Boolean(new Date(init).getDay()==7))
...
export function getDaysOfFDS (init: string, end: string):number {
  return getQtdOfWeeks(init, end) + getMajorDayOfWeek(init, end) * 2 + getSunday(init)
}
